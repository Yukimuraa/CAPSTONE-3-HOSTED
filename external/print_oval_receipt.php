...
$page_title = "Print Oval Field Booking Receipt - CHMSU BAO";
$base_url = "..";

// Get booking ID
if (!isset($_GET['booking_id'])) {
    header("Location: oval.php");
    exit();
}
...
$stmt = $conn->prepare("SELECT b.*, u.name as user_name, u.email as user_email, u.organization 
                        ...
                        WHERE b.booking_id = ? AND (b.facility_type = 'oval' OR b.booking_id LIKE 'OVAL-%') AND b.user_id = ?");
...
// Extract cost breakdown (if exists)
...
<body>
    <div class="no-print">
        <button class="print-button" onclick="window.location.href='oval.php'">
            ← Back to Oval Field Reservations
        </button>
                    ...
                    <div class="form-middle">
                        ...
                        <div style="margin-top: 4px;">
                            ...
                            <div class="form-value-large">
                                <?php 
                                $payment_nature = [];
                                $payment_nature[] = "Oval Field Reservation";
                                ...
                                if (!empty($additional_info['contact_person'])) {
                                    $payment_nature[] = "Contact Person: " . htmlspecialchars($additional_info['contact_person']);
                                }
                                if (!empty($additional_info['contact_number'])) {
                                    $payment_nature[] = "Contact Number: " . htmlspecialchars($additional_info['contact_number']);
                                }
                                echo implode("\n", $payment_nature);
                                ?>
                            </div>
                    ...
                    <div class="form-right">
                        ...
                        <div style="display: flex; flex-direction: column; gap: 2px;">
                            <?php if (!empty($cost_breakdown)): ?>
                                <?php if (isset($cost_breakdown['oval_field']) && $cost_breakdown['oval_field'] > 0): ?>
                                <div style="border-bottom: 1px solid #000; padding: 2px 3px; text-align: right; min-height: 14px; font-size: 6.5pt;">
                                    ₱<?php echo number_format($cost_breakdown['oval_field'], 2); ?>
                                </div>
                                <?php endif; ?>
                            <?php else: ?>
                                <div style="border-bottom: 1px solid #000; padding: 2px 3px; text-align: right; min-height: 14px; font-size: 6.5pt;">
                            ...
                            <?php 
                            $item_count = 0;
                            if (!empty($cost_breakdown) && isset($cost_breakdown['oval_field']) && $cost_breakdown['oval_field'] > 0) {
                                $item_count = 1;
                            } else {
                                $item_count = 1;
                            }
